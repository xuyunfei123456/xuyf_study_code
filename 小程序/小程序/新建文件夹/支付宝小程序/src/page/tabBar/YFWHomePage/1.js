...
export default class YFWHome extends Component {
  ...
  onTouchMove(e){
    if(this.scrollTop == 0 || this.state.touchHeight>0){
      let detail = e.touches[0]
      this.firstmove =  this.firstmove || detail.clientY
      let secondMove = detail.clientY,touchHeight = (secondMove-this.firstmove);
      touchHeight = touchHeight > 0 ? touchHeight:0
      this.setState({
        touchHeight,
        scrollY:false,
      })
      console.log('啤酒',detail)
    }
  }
  onTouchEnd(){
    this.firstmove="";
  ...
  getSettingInfo() {
    ...
    if (process.env.TARO_ENV == "weapp") {
      Taro.getSetting({
        success: (res) => {
          // scope.userLocation undefined==未请求过授权 true==地理位置已授权  false==已拒绝授权
          if (
  ...
  getLocal(latitude, longitude) {
    ...
    if (process.env.TARO_ENV == "weapp") {
      ...
      let success = function (data) {
        ...
        setGlobalData("city", addressInfo.addressComponent.city);
        setGlobalData("longitude", addressInfo.location.lng);
        setGlobalData("latitude", addressInfo.location.lat);
        setGlobalData("address", addressInfo.formatted_address);
        console.log(addressInfo.formatted_address);
        that.setState({
      ...
      // let _obj=[
      //   'ak=5Cp4gPP8c0nA0wj2oG25VRRAidrmMSuy',
      //   'output=json',
      //   'coordtype=wgs84ll',
      //   'location='+latitude+','+longitude
      // ]
      // Taro.request({
      //   url: 'http://api.map.baidu.com/reverse_geocoding/v3/?'+_obj.join('&'),
      //   method: 'GET',
      //   success:res=>{
      //     if(res&&res.data&&res.data.status === 0 ){
      //       let _res = res.data.result
      //       that.setState({
      //         locationInfo: _res.formatted_address,
      //       });
      //     }
          
      //   },
      //   fail:err=>{
      //     console.log('百度地图地址逆解系失败:'+err)
      //   }
      // })
    }
  }
  ...
  render() {
    ...
    return (
      <Block>
        ...
        <View style={{height:touchHeight}}>
          <Image  style="width:100%;height:100%" src={require('../../../images/loading_bg.png')} mode="widthFix"></Image>
        </View>
        <ScrollView
