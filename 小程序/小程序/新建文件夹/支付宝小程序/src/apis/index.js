...
import { handleResponse } from "../utils/responseHandle.js";
...
class PublicApi extends HTTP {
  /*根据经纬度获取地址*/
  getAddressByLatAndLng(data){
    return this.get("guest.common.app.getAddressByLat_lng",data)
  }
  /**获取区域ID */
...
class IndexApi extends HTTP {
  ...
  // 获取新版首页数据
  getIndexData() {
    return httpRequest.getHandle("guest.common.app.getIndexData_2021New")
  }
  /**获取推荐商品*/
...
class GoodsDetailApi extends HTTP {
  ...
  getShopAndPlatformQualification(shopInfos, platformInfos) {
    ...
    tasks.push({
      ...
      param: { storeid: shopInfos.shopID },
    });
    tasks.push({
...
class ShopDetailApi extends HTTP {
  ...
  getShopGoods(shopID, categaryID, sortType, pageIndex, keyWords) {
    ...
    return httpRequest.getHandle(
      "guest.shopMedicine.getStoreMedicineSearch",
      params
    );
  }
}
...
class GoodsCategaryApi extends HTTP {
  ...
  getAllBrands(categoryID) {
    var conditions = { categoryid: categoryID };
...
class UserCenterApi extends HTTP {
  ...
  updateUserName(name) {
    return this.get("person.account.update_app", {
      data: JSON.stringify({ real_name: name }),
    });
  }
  /**更新QQ*/
  updateUserQQ(qq) {
  ...
  getMyComplaints(pageIndex) {
    return this.get("person.complaints.getPageData", {
      pageIndex: pageIndex,
      pageSize: 10,
      conditions: JSON.stringify({ type: "-1" }),
    });
  }
  /**我的收藏--商品*/
  getMyCollectionGoods(pageIndex) {
    return httpRequest.getHandle(
      "person.favorite.getUserCollectionStoreGoods",
      {
        pageIndex: pageIndex,
        pageSize: 20,
      }
  ...
  getMyCoupons(pageIndex, type,couponType) {
    ...
    if(couponType){
      parmas.dict_coupons_type=couponType+''
    }
    return this.get("person.usercoupons.getPageData", parmas);
...
class OrderApi extends HTTP {
   ...
   */
  complaintsToTheBusinessman(orderID, type, content, voucherImageUrls) {
    var params = {
      orderno: orderID,
      type: type,
      content: content,
    ...
    if (voucherImageUrls) {
      params.introImage = JSON.stringify(voucherImageUrls);
    }
    return this.get("person.order.complaint", params);
  }
  /**投诉详情
   */
   ...
   */
  getLogisticsDetails(orderID, isRefresh) {
    var params = {
      orderno: orderID,
    };
    if (isRefresh) {
    ...
    return httpRequest.getHandle("person.order.getShippingTrace", params);
  }
  /**订单评价
   * servicesScore 服务评分
   * sendScore 发货速度评分
  ...
  applyForRefundGoods(
    ...
    return this.get("person.order.applyReturnGoods", {
      orderno: orderID,
      reason: reason,
      image_voucher: JSON.stringify(voucherImageUrls),
      image_report: JSON.stringify(reportImageUrls),
      money: money,
      returnGoodsInfo: JSON.stringify(returnGoodsArray),
    });
  }
  /**申请退货--填写物流信息
   * trafficNo 物流单号
...
class HealthAskApi extends HTTP {
  ...
  getPharmacistAnswerList(pharmacistID, pageIndex) {
    return this.get("guest.ask.getDoctorDetailPageData", {
      conditions: { pharmacistid: pharmacistID },
      pageIndex: pageIndex,
      pageSize: 20,
    });
  }
...
class SaleComparePricesApi extends HTTP {
  getGoodsAndShopData(conditions, pageIndex) {
    var tasks = new Array();
    tasks.push({
      name: "goodsInfo",
      cmd: "guest.medicine.getMedicineDetail",
      param: {
        mid: conditions.medicineid,
      },
    });
    tasks.push({
      name: "shopsInfo",
...
class ShopCarApi extends HTTP {
   ...
   * packageId  套餐/疗程装ID，多个用,分割
   * isBuy  是否是提交需求
   */
  addGoodsToShopCar(
    quantity,
    storeMedicineId = null,
    packageId = null,
    isBuy = false
  ) {
    let params = {
  ...
  getShopCarCount() {
    return new Promise((resolve, reject) => {
      this.get("person.cart.getCartCount").then(
        (result) => {
          Taro.setStorageSync("shopCarCount", result.cartCount);
          resolve(result);
        },
        (error) => {
          reject(error);
   ...
   * packageIds 套餐或疗程装在购物车里的ID 多个用,分割
   */
...
class OrderPaymentApi extends HTTP {
   ...
   * info = {
            ...
            packageids:tcpPackageIds,//套餐或疗程装在购物车里的ID  多个用,分割
   ...
   */
  ttOrderPay(orderno) {
    return this.get("guest.common.app.getToutiaoPayInfo", {
      orderno: orderno,
    });
  }
   ...
   * 支付宝订单生成
   */
  alipayOrderPay(orderno) {
    return this.get("guest.common.app.getBatchOrderInfo", {
      type: "alipay",
      ordernoList: orderno,
    });
  }
   ...
   */
  qqOrderPay(orderno) {
    return this.get("guest.common.app.getOrderQQPayInfo", {
      orderno: orderno,
    });
  }
      ...
      Taro.login({
        success(res) {
          if (res.code) {
            ...
            reject({ errMsg: "获取授权信息失败" });
          }
        },
      });
    })
      .then((code) =>
        this.get("guest.weixin.getOpenid", {
          code: code,
        })
      )
      .then((openid) =>
        this.get("guest.order.getPayInfo", {
          type: "wxpay",
          client: "h5_wx",
          openid: openid,
          orderno: orderno,
...
class UseDrug extends HTTP {
  ...
  getDiseaseAboutKeyword(keywords, type) {
    let params = {
      ...
      url =
        type == "gmDiseaseData"
          ? "guest.allergy.getListByKeywords"
          : "guest.disease.getListByKeywords";
