...
function setItemModel(data, froms) {
  ...
  let goods_Type = '',troche_type = '',companyName = ''
  ...
  if (froms === 'cart_list_recommend') {
    ...
    var appSystemConfig = getGlobalData('appSystemConfig')
    /** 商家内全部商品搜索 */
    ...
    isShowCart = (parseInt(appSystemConfig.wx_rx_is_buy) != 0 || parseInt(data.prescriptionType) < 0) && data.is_add_cart && isLogin()
    isShowStories = false
    navitation_params = {
    ...
    troche_type = data.originData.troche_type
    companyName = data.originData.mill_title
    ...
    troche_type = data.originData.trocheType
    companyName = data.originData.title
    goods_id = data.originData.store_medicineid
    ...
    goods_standard = safe(data.standard)
    goods_stories = safe(data.store_count)
    store_title = isNotEmpty(data.store_title)
    data.originData.applicability = data.originData.applicability.replace(/<[^>]+>/g, "").replace(/(↵|\r|\n|&|=|\?)/g, "").trim()
    navitation_params = {
      type: isNotEmpty(data.store_title) ?'get_shop_goods_detail':'get_goods_detail',
      value: goods_id,
  ...
  return {
    ...
    store_title: isNotEmpty(store_title)?store_title:'',
