    ...
    if (isNotEmpty(data)) {
      ...
      data.dataList.forEach((item, index) => {
        ...
        if (isNotEmpty(item.medicine_list) && item.medicine_list.length > 0) {
          item.medicine_list.forEach((medicineitem, index) => {
            normalMedicines.push({
              id: medicineitem.id,
              activity_img_url: safe(medicineitem.activity_img_url),
        ...
        if (isNotEmpty(item.packmedicine_list)) {
          item.packmedicine_list.forEach((packmedicine, index) => {
            ...
            medicine.push({
              ...
              price: toDecimal(packmedicine.smp_price),
        ...
        returnArray.push({
          ...
          condition_price: Number.parseFloat(item.condition_price),
          sub_price: Number.parseFloat(item.sub_price),
        });
      });
    }
    return returnArray;
